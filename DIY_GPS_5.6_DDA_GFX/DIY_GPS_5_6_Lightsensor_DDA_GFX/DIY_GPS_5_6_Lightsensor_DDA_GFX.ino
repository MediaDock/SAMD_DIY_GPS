...
void setup() {
  ...
  if (csvFile) {
    csvFile.print("Date,Time,Longitude,Latitude,Altitude,Temperature,Humidity,LightOutput,minLightOutput,maxLightOutput");
    csvFile.println();    
  ...
  display.println(F("GPS_5.6_LIGHTSENSOR"));
  display.display();
...
Serial.println(light_value);
   ...
   // Check if the current LightOutput is higher or lower than the previous values
   if (light_value > maxLightOutput) {
       maxLightOutput = light_value; // Update maxLightOutput if current LightOutput is louder
   }
   if (light_value < minLightOutput) {
       minLightOutput = light_value; // Update minLightOutput if current LightOutput is quieter
   }

    lightMapped = map(light_value, 0, 1023, 0, 100);


   // Print the max and min microphone output values
   Serial.print("Max Light Output: ");
   Serial.println(maxLightOutput);
   Serial.print("Min Light Output: ");
   Serial.println(minLightOutput);
  delay(200);
}   
...
display.fillRect(23, 36, lightMapped, 15, 1);
display.setTextWrap(false);
display.setCursor(23, 54);
display.setTextSize(1);
display.print(F("Licht"));
display.drawBitmap(4, 36, image_light_bits, 15, 16, 1);
...
void allDATA_log()  { /* SD Card LOG on Update -> activated in line 470 ************************************************/
  ...
  if (myFile)
  {
    ...
    /* allDATA: Lightsensor                      **************************************/
    myFile.print(", licht: "); 
    myFile.print(light_value);
    myFile.print(", minLightOutput: ");
    myFile.print(minLightOutput);
    myFile.print(", maxLightOutput");
    myFile.println(maxLightOutput);
...
void CSV_log()  { /* SD Card LOG on Update -> activated in line 503 ************************************************/
  ...
  if (csvFile) {
    ...
    csvFile.print(HUMID);
    csvFile.print(",");
    csvFile.print(light_value);
    csvFile.print(",");
    csvFile.print(minLightOutput);
    csvFile.print(",");
    csvFile.print(maxLightOutput);
    csvFile.println();
    csvFile.close(); // close the file
